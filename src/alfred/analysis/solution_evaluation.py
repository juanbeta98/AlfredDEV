...
def _get_labors(svc: Dict[str, Any]) -> List[Dict[str, Any]]:
    """Return labors for a service, accepting both snake_case and camelCase key."""
    return svc.get("service_labors") or svc.get("serviceLabors") or []
...
def filter_by_date(
    ...
    """Keep only labors scheduled on planning_date (YYYY-MM-DD); drop empty services."""
    cutoff = _date.fromisoformat(planning_date)
    result: List[Dict[str, Any]] = []
    dropped = 0
    for svc in services:
        kept = []
        for lab in _get_labors(svc):
            ...
            kept.append(lab)
        if kept:
            _lk = "service_labors" if "service_labors" in svc else "serviceLabors"
            result.append({**svc, _lk: kept})
    if dropped:
        print(f"[filter_by_date] Dropped {dropped} labor(s) not on {planning_date}.")
    return result
...
def build_preassigned_lookup(input_services: List[Dict[str, Any]]) -> Dict[Any, Any]:
    """Return {labor_id: original_alfred} from an input snapshot.

    In the input payload, ``alfred`` is a raw driver id (int or str) when the
    labor was preassigned, and ``null`` otherwise.  This lookup lets
    ``flatten_labors`` populate ``original_assigned_driver`` even for older
    output payloads that predate the pipeline fix.
    """
    lookup: Dict[Any, Any] = {}
    for svc in input_services:
        for labor in _get_labors(svc):
            lid = labor.get("id")
            alfred = labor.get("alfred")
            if lid is not None and alfred is not None:
                lookup[lid] = alfred
    return lookup
...
def flatten_labors(
    ...
    preassigned_lookup: Optional[Dict[Any, Any]] = None,
    ...
    for svc in services:
        ...
        for lab_idx, labor in enumerate(_get_labors(svc)):
            labor_id = labor.get("id")
            add_data: Dict[str, Any] = labor.get("addData") or {}
            _alfred = labor.get("alfred")
            driver_id = _alfred.get("id") if isinstance(_alfred, dict) else _alfred
            ...
            has_routing_endpoints = bool(add_data.get("map_start_point"))
            if has_routing_endpoints and reported_dist is not None:
                # Algorithm provided its own routing endpoints — its reported distance
                # is authoritative (driver's actual path, not service start→end geometry).
                labor_distance_km: float = float(reported_dist)
            elif coord_lookup is not None and labor_id in coord_lookup:
...
def compute_payload_summary(
    ...
    assigned_count = infeasible_count = reassignment_count = vt_count = preassigned_count = 0
    in_grace_count = 0
    ...
    for r in rows:
        ...
        if r.get("original_assigned_driver") is not None:
            preassigned_count += 1
    ...
    return {
        ...
        "labors_preassigned":              preassigned_count,
