...
from alfred.data.geo.location import parse_service_location
...
class InputParser:
    ...
    def parse(json_data: Dict[str, Any]) -> Tuple[pd.DataFrame, Dict[str, Any]]:
        ...
        for service in services:
            ...
            labors = service.get("service_labors") or service.get("serviceLabors") or []
            if not isinstance(labors, list):
                logger.warning(
                    "Invalid service_labors format",
        ...
        record.update(
            {
                ...
                "labor_type": InputParser._derive_labor_type(labor),
    ...
    @staticmethod
    def _derive_labor_type(labor: Dict[str, Any]) -> Optional[str]:
        """
        Return the labor type as a lowercase-underscore slug (e.g. 'alfred_initial_transport').

        Preference order:
        1. labor_name — already present as a human-readable string; slug it.
        2. labor_type — used when the payload already carries the slug (e.g. algorithm output).
        3. labor_id (integer) — last resort; returned as a string so downstream code
           always receives a consistent type, but will not match named sets like _VT_LABOR_TYPES.
        """
        name = labor.get("labor_name")
        if name:
            return str(name).strip().lower().replace(" ", "_")
        lt = labor.get("labor_type")
        if lt is not None:
            return str(lt)
        lid = labor.get("labor_id")
        if lid is not None:
            return str(lid)
        return None
